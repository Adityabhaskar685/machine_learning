...
class ConditionStep:
    ...
    Args: 
        criteria (float) : Coefficient applied to the metric of the model registerd in the model registry.
        metric (str): Metric as a reference . Can be ['precision', 'recall', 'roc_auc"].
    """

    def __init__(self, criteria : float, metric : Literal['roc_auc', 'precision', 'recall']) -> None:
    ...
    def __call__(self, mlflow_run_id: str) -> None:
        ...
        if `metric_run > registered_metric* (1 + self.criteria)`, then the model is registed.
        """
        ...
        if not registered_models:
            ...
            LOGGER.info("New model registerd")
