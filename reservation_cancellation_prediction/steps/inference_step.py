...
class InferenceStemp:
    """Get the model from model registry and predict in batch"""

    def __call__(self, batch_path : Path) -> List[int]:
        """Use the MLFlow artifact build-in predict.
    ...
    def _load_model(registered_model_name: str):
        ...
        if models:
            latest_model_version = models[0].latest_versions.version
            LOGGER.info(
